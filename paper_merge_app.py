...
DATA_DIR = 'input_data'
FILE_7000 = os.path.join(DATA_DIR, '0201.csv')
FILE_INPUT = os.path.join(DATA_DIR, '0201.csv')
...
def consolidate_row(row):
    ...
    if is_empty(row.get('isbn')):
        for c in ['bookscom_isbn', 'eslite_isbn', 'kingstone_isbn', 'sanmin_isbn']:
            if not is_empty(row.get(c)):
                row['isbn'] = row[c]
                break
    # Title (Standard Title)
    if is_empty(row.get('title')):
        for c in ['bookscom_title', 'eslite_title', 'kingstone_title', 'sanmin_title']:
            if not is_empty(row.get(c)):
                row['title'] = row[c]
                break
    ...
    if is_empty(row.get('processed_title')):
        for c in ['bookscom_processed_title', 'eslite_processed_title', 'kingstone_processed_title', 'sanmin_processed_title']:
            if not is_empty(row.get(c)):
    ...
    if is_empty(row.get('original_title')):
        for c in ['bookscom_original_title', 'eslite_original_title', 'kingstone_original_title', 'sanmin_original_title']:
            if not is_empty(row.get(c)):
                row['original_title'] = row[c]
                break
    # Author
    if is_empty(row.get('author')):
        for c in ['bookscom_author', 'eslite_author', 'kingstone_author', 'sanmin_author']:
            if not is_empty(row.get(c)):
                row['author'] = row[c]
                break
    # Translator
    if is_empty(row.get('translator')):
        for c in ['bookscom_translator', 'eslite_translator', 'kingstone_translator', 'sanmin_translator']:
            if not is_empty(row.get(c)):
                row['translator'] = row[c]
                break
    # Publisher
    if is_empty(row.get('publisher')):
        for c in ['bookscom_publisher', 'eslite_publisher', 'kingstone_publisher', 'sanmin_publisher']:
            if not is_empty(row.get(c)):
                row['publisher'] = row[c]
                break
    return row
...
def merge_two_books(book1, book2):
    ...
    for col in ['bookscom_publish_date', 'eslite_publish_date', 'kingstone_publish_date', 'sanmin_publish_date']:
        for book in [book1, book2]:
            val = book.get(col)
            # print(f"[DEBUG] Checking date col {col}: {val}") # Uncomment if needed
...
def merge_data():
    ...
    is_numeric_ids = True
    try:
        # Check compatibility with int but use string for comparison
    ...
    # using global consolidate_row function
...
def unmerge_data():
    ...
    try:
        ...
        # Use string comparison for index since df is loaded as string
        merge_position = df[df['index'].astype(str) == target_str].index
        
        if len(merge_position) > 0:
            target_idx = target_id # For logging purpose
            ...
            for col in prod_cols:
                ...
                if val and val.lower() != 'nan':
                    ...
                    # Clean IDs: remove .0 suffix if present (common artifact from float conversion)
                    ...
                    if p_ids:
                        ...
                        print(f"[DEBUG] Found IDs in {col}: {p_ids}")
            ...
            if has_valid_id:
                ...
                original_rows = input_df[mask].copy() # Add .copy() to avoid SettingWithCopyWarning
                
                if original_rows.empty:
                        return jsonify({'error': 'No original data found for the extracted IDs'}), 404
                
                # ✅ 對還原後的原始資料執行 Consolidate，填補空的大欄位
...
def create_row():
    ...
    # (We set a temp index here, it will be reindexed anyway)
    new_row = {'index': -1}
    for col in df.columns:
    ...
    if insert_after is not None:
        try:
            # Find the position of the insert_after index
            # Ensure index comparison is type safe
            ...
            if matches:
                ...
                # Fallback to append if index not found
